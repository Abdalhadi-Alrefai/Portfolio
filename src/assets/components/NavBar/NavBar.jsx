...
const NavBar = ({ logo, navItems }) => {
  ...
  useEffect(()=> {
    const ClickOut = (event) => {
      if (
        sideBar.current && !sideBar.current.contains(event.target) && toggleBtn.current && !toggleBtn.current.contains(event.target)
      ) {
        setIsOpen(false)
      }
    }
  ...
  return (
    ...
    >
      ...
      >
        {isOpen ? <img src="/assets/img/Close.svg"/>  : <img src="/assets/img/Menu.svg"/>}
      </button>
